...
public class CellPosition {
  ...
  // note: ValueObject는 equals와 hashCode를 구현하는 습관을 기르는것이 좋다.
  @Override
  public int hashCode() {
  ...
  public CellPosition calculatePositionBy(RelativePosition relativePosition) {
    // note: minesweeper 로직에서 canCalculatePositionBy가 이 메서드 이전에 호출되지만
    // note: 이 메서드를 사용하는 입장에서 canCalculatePositionBy 이전에 호출하는것을 깜빡할 여지가 많기에
    // note: 안정적인 메서드 구현을 위해서 이 메서드 안쪽에서 이것을 호출한다.
